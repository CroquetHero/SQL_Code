select issuercountry || '-' || paymentmethodid || '-' || card_source || '-' || card_class Country_Source
,bin
,dateincrement
...
group by bin
,dateincrement
--,(case when order_date <= '9/12/2018' then 'Pre' else 'Post' end)
--,paymentmethodid
--,issuercountry
,newstatus
,issuercountry || '-' || paymentmethodid || '-' || card_source || '-' || card_class
