...
select subscription_id
    --  ,product_data_id
      ,(case when (select fdoa.status from frd_dispute_order_activity fdoa, sub_subscription_li_item sslia, req_line_item rlia where sslia.req_line_item_id = rlia.line_item_id and rlia.requisition_id = fdoa.requisition_id and fdoa.status = 'STATUS_DISPUTE_CHARGE_BACK') is not null then 'CB' else 'No CB' end)cb_status
...
and ssli.creation_date between '01-sep-12' and '01-oct-12'
